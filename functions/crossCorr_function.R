...
if (testsum > 0) {
 ...
 # indMin = match(minimum, ccvalues, nomatch = NA_integer_, incomparables = NULL)
  ...
  plot(time,y1,type="l",col="blue", xlab ="time", ylab = "Raw Data", main = "Raw Data of 2 channels")
  lines(time,y2,col="black") #--> works!
  
  
  # PLOT 2. Cross-correlation function - ccPrint on x-axis, ccvalues on y-axis
  plot(ccLagsPrint,ccvalues,type="l",col="red" , xlab ="time Lag", ylab = "Cross-correllation values", main = "Cross-correlated vales") # --> works awesomely!!!
  
  
  # PLOT 3. Time series 2 and shift-corrected time series 1
  plot(time,y1, type="l", col="blue", lwd=1, xlab ="time", ylab = "shifted Data", main = "Raw and Shifted Data")
