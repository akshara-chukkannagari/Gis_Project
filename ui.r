...
shinyUI(fluidPage(theme = shinytheme("cerulean"),
  ...
  sidebarPanel (
    ...
    checkboxInput('file_check', 'Click and upload file', F),
    fileInput("file","upload a file"),
    helpText("Max. file size is 5MB"),
    
    uiOutput("datasize"),
    
    dateInput('date_start',label = 'Start Date',"2014-11-24",format = "yyyy-mm-dd"),#start = Sys.Date() - 2, end = Sys.Date() + 2),
    dateInput('date_end',label = 'End Date',"2014-11-26",format = "yyyy-mm-dd"),
    textInput("time_start","Start Time", "10:51:31"),
    textInput("time_end","End Time", "10:51:31"),
  ...
  mainPanel(
    tabsetPanel(type="tab",
                ...
                tabPanel("Data",downloadButton("data_down","Download Data"), tableOutput("data")),
                tabPanel("Statistics",h4(textOutput("samplesize")), verbatimTextOutput("stats"), uiOutput("raw_data"), h3(textOutput("st_text")), h3(textOutput("st_test")), plotOutput("plot_temp")),
                                     ...
                                     tabPanel("ACF", 
                                              fluidRow (style= "padding-bottom: 20px",
                                                        ...
                                                        column(3, radioButtons(
                                                          ...
                                                          selected="covariance"))),
                                              fluidRow(plotOutput("plot_acf"), downloadButton(outputId="down_acf",label="Download Plot"))),
                                     tabPanel("CCF", 
                                              fluidRow (style= "padding-bottom: 20px",
                                                        ...
                                                        width=2),
                                              fluidRow(plotOutput("plot_ccf"), downloadButton(outputId="down_ccf",label="Download Plot")))                    
                                          )),
...
fluidRow(helpText('Click coefficient test button to view the test results and then download the results as csv file')),
fluidRow(actionButton(inputId="coef_test", label="Coefficient testing"),tableOutput("coef_table"),downloadButton("coef_down","Download coef test results")),

                         fluidRow(plotOutput("plot_reg"), downloadButton(outputId="down_reg",label="Download Plot"))),
